...
def make_url_for(p): return ''.join(pattern2regexp(
    p, lambda v, r: '%%(%s)s' % v, lambda s: s))


def make_variables(p): return filter(
    lambda x: x, pattern2regexp(p, lambda v, r: v, lambda s: None))


def make_pattern(p): return r''.join(
    pattern2regexp(p, lambda v, r: r'(?P<%s>%s)' % (v, r)))


def make_selector_fragment(p): return r''.join(
    pattern2regexp(p, lambda v, r: r'(?:%s)' % r))


def make_selector(i): return r'(^%s$)' % r'$)|(^'.join(
    map(make_selector_fragment, i))
...
class UrlMap(object):
    ...
    def add(self, pattern, endpoint, **kw):
        ...
        self._endpoints[(endpoint, frozenset(
        ...
        if keyvalues != frozenset():
            if self._defaults[endpoint].has_key(keyvalues):
                raise Exception(
                    'duplicate defaults for endpoint', endpoint, kw)
        ...
        self._patterns[pattern] = (re.compile(
            make_pattern(pattern)), endpoint, kw)
        self._pattern_selector = re.compile(
            make_selector(self._patterns.iterkeys()))
    ...
    def route(self, url):
        ...
        """
        try:
            p = self._patterns.values()[re.match(
                self._pattern_selector, url).lastindex - 1]
            m = p[0].match(url)
            d = m.groupdict().copy()
            d.update(p[2])
